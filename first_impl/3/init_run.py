...
def run():
    class MyPytestPlugin:
        ...
        def init_db(self):
            self.cursor.execute('CREATE TABLE test_map (file_id INTEGER, test_function_id INTEGER, line_id INTEGER, UNIQUE(file_id,test_function_id,line_id))')
            self.cursor.execute('CREATE TABLE src_file (id INTEGER PRIMARY KEY, path TEXT, UNIQUE (path))')
            self.cursor.execute('CREATE TABLE test_file (id INTEGER PRIMARY KEY, path TEXT, UNIQUE (path))')
            self.cursor.execute('''CREATE TABLE test_function (
