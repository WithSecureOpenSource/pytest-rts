...
def git_diff_data(filename,commithash1,commithash2):
    ...
    git_data = str(subprocess.run(['git','--git-dir',git_dir,'diff','-U0',commithash1,commithash2,'--',filename],stdout=PIPE, stderr=PIPE).stdout)
    return git_data
...
def tests_from_changes(commithash1,commithash2):
    ...
    test_test_set,test_changed_lines_dict,test_new_line_map_dict = tests_from_changed_testfiles(changed_test_files,commithash1,commithash2)
    src_test_set,src_changed_lines_dict,src_new_line_map_dict = tests_from_changed_sourcefiles(changed_source_files,commithash1,commithash2)

    test_set = test_test_set.union(src_test_set)
    update_tuple = (test_changed_lines_dict,test_new_line_map_dict,src_changed_lines_dict,src_new_line_map_dict)
...
def get_testfiles_and_srcfiles():
    conn = sqlite3.connect('example.db')
    c = conn.cursor()
    test_files = [(x[0],x[1]) for x in c.execute('SELECT id,path FROM test_file').fetchall()]
    src_files = [(x[0],x[1]) for x in c.execute('SELECT id,path FROM src_file').fetchall()]
    conn.close()
...
def file_changes_between_commits(commit1, commit2):
    git_dir = './'+PROJECT_FOLDER+'/.git'
    git_data = subprocess.run(['git','--git-dir',git_dir,'diff','--name-only',commit1,commit2],stdout=PIPE, stderr=PIPE).stdout
...
def commits_test():
    ...
    for commit in git_commits[start+1:end]:
        ...
        subprocess.run(['python','collector.py',PROJECT_FOLDER])
...
def random_remove_test(iterations):
    ...
    c = conn.cursor()
    c.execute('CREATE TABLE IF NOT EXISTS data (specific_exit INTEGER, all_exit INTEGER)')
    ...
    for i in range(iterations):
        ...
        git_data = str(subprocess.run(['git','diff','-U0',src_name],stdout=PIPE, stderr=PIPE).stdout)
        os.chdir('..')
        test_lines, updates_to_lines = get_test_lines_and_update_lines(git_data)
        tests = query_tests_sourcefile(test_lines,src_id)

        try:
            specific_exit_code = int(str(subprocess.run(['python','specific_without_remap.py',PROJECT_FOLDER]+tests,capture_output=True,timeout=60).stdout,'utf-8').strip())
        except subprocess.TimeoutExpired:
            specific_exit_code = -1

        try:
            all_exit_code = int(str(subprocess.run(['python','all_without_remap.py',PROJECT_FOLDER],capture_output=True,timeout=60).stdout,'utf-8').strip())
        except subprocess.TimeoutExpired:
            all_exit_code = -1
...
def main():
    ...
    ans = input('which test? [1]: commits [2]: random_remove')
    if ans == '1':
        commits_test()  
    elif ans == '2':
