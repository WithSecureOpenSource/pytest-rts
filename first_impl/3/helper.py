...
def query_tests_testfile(lines_to_query,file_id):
    ...
    for line_id in lines_to_query:
        data = cursor.execute('''SELECT DISTINCT context FROM test_function
                                WHERE test_file_id = ? 
                                AND start <= ?
                                AND end >= ?''',(file_id,line_id,line_id))
        for line in data:
            test = line[0]
            tests.append(test)
    conn.close()
...
def query_tests_sourcefile(lines_to_query,file_id):
    ...
    for line_id in lines_to_query:
        data = cursor.execute('''SELECT DISTINCT context FROM test_function
                                ...
                                WHERE test_map.file_id = ? ''',(file_id,))
        for line in data:
            test = line[0]
            tests.append(test)
    conn.close()
...
def delete_ran_lines(line_ids,file_id):
    ...
    for line in line_ids:
        cursor.execute('DELETE FROM test_map WHERE line_id == ? AND file_id == ?',(line,file_id))
    conn.close()
...
def update_db_from_src_mapping(line_map,file_id):
    ...
    for line_id in line_map.keys():
        db_data = cursor.execute('SELECT file_id,test_function_id,line_id FROM test_map WHERE line_id == ? AND file_id == ?',(line_id,file_id))
        for line in db_data:
            tests_to_update.append(line)
        cursor.execute('DELETE FROM test_map WHERE line_id == ? AND file_id == ?',(line_id,file_id))
    for t in tests_to_update:
        updated_t = (t[0],t[1],line_map[t[2]])
        cursor.execute('INSERT OR IGNORE INTO test_map (file_id,test_function_id,line_id) VALUES (?,?,?)',updated_t)     
    conn.commit()
    conn.close()
...
def update_db_from_test_mapping(line_map,file_id):
    ...
    tests_to_update = []
    for line_id in line_map.keys():
        db_data = cursor.execute('''SELECT id,test_file_id,context,start,end FROM test_function
                                    WHERE test_file_id = ?''',(file_id,))
        for line in db_data:
            tests_to_update.append(line)
    ...
    for t in tests_to_update:
        ...
        cursor.execute('INSERT OR IGNORE INTO test_function (id,test_file_id,context,start,end) VALUES (?,?,?,?,?)',updated_t)     
    conn.commit()
    conn.close()
...
def start_test_init(PROJECT_FOLDER):
    ...
    os.rename(os.getcwd()+'/.coveragerc',os.getcwd()+'/'+PROJECT_FOLDER+'/.coveragerc')
    subprocess.run(['python','init_run.py',PROJECT_FOLDER])
    os.rename(os.getcwd()+'/'+PROJECT_FOLDER+'/example.db',os.getcwd()+'/example.db')
    os.rename(os.getcwd()+'/'+PROJECT_FOLDER+'/.coveragerc',os.getcwd()+'/.coveragerc')

def start_normal_phase(PROJECT_FOLDER,test_set):
    os.rename(os.getcwd()+'/example.db',os.getcwd()+'/'+PROJECT_FOLDER+'/example.db')
    if os.path.exists("./"+PROJECT_FOLDER+"/.coveragerc"):
        os.remove("./"+PROJECT_FOLDER+"/.coveragerc")
    os.rename(os.getcwd()+'/.coveragerc',os.getcwd()+'/'+PROJECT_FOLDER+'/.coveragerc')
    subprocess.run(['python','normal_run.py',PROJECT_FOLDER]+list(test_set))
